(() => {
    ...
    const render = (deps) => {
        ...
        const badge = (tone, text) => {
            const cls = tone === 'positive' ? 'positive' : tone === 'negative' ? 'negative' : tone === 'warn' ? 'warn' : 'neutral';
        ...
        const microOkHtml = `<span class="usop-pill">Micro OK ${deps.escapeHtml(String(model.edgeMicroOk || 0))}/3</span>`;
        ...
        el.innerHTML = `
            ...
            <details class="usop-details">
                ...
                <div style="margin-top:12px;padding:12px;border:1px solid rgba(255,255,255,.12);border-radius:14px;background:rgba(0,0,0,.18);">
                    <div style="font-weight:900;letter-spacing:.8px;opacity:.95;margin-bottom:8px;">EDGE (composição)</div>
                    <div style="opacity:.86;font-size:12px;line-height:1.35;">
                        base ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.base : 0))} +
                        microAlign ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.microAlign : 0))} +
                        netAvg ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.netAvg : 0))} +
                        regime ${deps.escapeHtml(fmt2(model.edgeParts ? (model.edgeParts.regimeScore - 0.5) : 0))} +
                        sessão ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.sessionAdj : 0))} +
                        vol ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.volAdj : 0))} +
                        trend ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.volTrend : 0))} +
                        stale ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.staleAdj : 0))} +
                        diverg ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.divAdj : 0))} +
                        modo ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.modeAdj : 0))}
                        = raw ${deps.escapeHtml(fmt2(model.edgeParts ? model.edgeParts.raw : 0))} → EDGE ${deps.escapeHtml(String(model.edgePct))}%
                    </div>
                    <div style="margin-top:8px;opacity:.78;font-size:12px;line-height:1.35;">
                        Micro OK conta quantos índices (SPX/NDX/DOW) estão com tendência curta validada (5m×15m) e força suficiente para o regime de vol.
                    </div>
                </div>
                <div style="margin-top:12px;padding:12px;border:1px solid rgba(255,255,255,.12);border-radius:14px;background:rgba(0,0,0,.18);">
                    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
