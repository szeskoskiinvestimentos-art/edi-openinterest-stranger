(() => {
    const h = (() => {
        try { return (typeof window !== 'undefined' && window.RatesBucketsHelpers) ? window.RatesBucketsHelpers : {}; } catch { return {}; }
    })();
    ...
    const render = ({ data, el, deps } = {}) => {
        ...
        const buildDiList = typeof h.buildDiList === 'function'
            ? h.buildDiList
            : (({ d, deps, pointPct }) => ({ diSymbolsAll: [], diList: [] }));
        const bucketAvgYield = typeof h.bucketAvgYield === 'function' ? h.bucketAvgYield : (() => null);
        const bucketAvgMove = typeof h.bucketAvgMove === 'function' ? h.bucketAvgMove : (() => null);
        const maturityYears = typeof h.maturityYears === 'function' ? h.maturityYears : (() => null);
        const bucketOfYears = typeof h.bucketOfYears === 'function' ? h.bucketOfYears : (() => 'Curto');

        const diPack = buildDiList({ d, deps, pointPct });
        const diSymbolsAll = diPack && Array.isArray(diPack.diSymbolsAll) ? diPack.diSymbolsAll : [];
        const diList = diPack && Array.isArray(diPack.diList) ? diPack.diList : [];
