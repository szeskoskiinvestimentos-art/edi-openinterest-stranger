...
function renderMarketPanorama(data) {
    ...
    for (const g of groups) {
        ...
        for (const r of rows) {
            const s = String(r && r.symbol ? r.symbol : '');
            if (s) visibleSet.add(s);
        }
    }
    const hiddenByCut = Array.from(monitoredUnique).filter(s => panoramaSet.has(s) && !visibleSet.has(s));

    const coverageHtml = `
        <div style="border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:12px;background:rgba(0,0,0,.18);margin-bottom:12px;">
            <div style="display:flex;align-items:baseline;justify-content:space-between;gap:12px;flex-wrap:wrap;">
                <div style="font-weight:900;letter-spacing:1px;opacity:.95;">Cobertura do Panorama</div>
                <div style="opacity:.80;font-size:12px;">Ativos: ${escapeHtml(String(monitoredUnique.size))} • Exibidos agora: ${escapeHtml(String(visibleSet.size))} • No panorama: ${escapeHtml(String(inPanoramaCount))}${extrasInPanorama ? ` • Extras: ${escapeHtml(String(extrasInPanorama))}` : ''}${duplicates ? ` • Duplicados: ${escapeHtml(String(duplicates))}` : ''}</div>
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;align-items:center;justify-content:space-between;">
                <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
                    ...
                    <span class="${hiddenByCut.length ? 'neutral' : 'positive'}" style="display:inline-flex;align-items:center;gap:8px;border:1px solid rgba(255,255,255,.14);border-radius:999px;padding:4px 10px;background:rgba(0,0,0,.18);font-family:'Share Tech Mono',monospace;font-weight:900;">Ocultos (corte): ${escapeHtml(String(hiddenByCut.length))}</span>
                </div>
                <div style="display:flex;gap:10px;align-items:center;">
                    <button class="panorama-freeze" data-panorama-expand-all="1">Expandir tudo</button>
                    <button class="panorama-freeze" data-panorama-collapse-all="1">Recolher tudo</button>
                </div>
            </div>
            ${missingInPanorama.length
        ? `<details style="margin-top:10px;">
                <summary style="cursor:pointer;opacity:.90;font-weight:900;">Faltando (${escapeHtml(String(missingInPanorama.length))})</summary>
                <div style="margin-top:8px;opacity:.88;line-height:1.35;">${escapeHtml(missingInPanorama.join(' • '))}</div>
            </details>`
        : ''}
            ${hiddenByCut.length
        ? `<details style="margin-top:10px;">
                <summary style="cursor:pointer;opacity:.90;font-weight:900;">Ocultos pelo corte (${escapeHtml(String(hiddenByCut.length))})</summary>
                <div style="margin-top:8px;opacity:.88;line-height:1.35;">${escapeHtml(hiddenByCut.join(' • '))}</div>
            </details>`
        : ''}
        </div>
    `;
    ...
    const expandAllBtn = el.querySelector('[data-panorama-expand-all]');
    if (expandAllBtn) {
    ...
    const collapseAllBtn = el.querySelector('[data-panorama-collapse-all]');
    if (collapseAllBtn) {
