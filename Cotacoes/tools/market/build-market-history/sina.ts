import { mergeSinaQuoteIntoMarketQuotes } from '../lib/market-quotes/merge.js'
import { fetchSinaHqQuote } from '../lib/market-quotes/sina-hq.js'

export async function tryMergeSinaDceI0(params: {
  enabled: boolean
  outDir: string
  ...
  try {
    const code = params.env('SINA_DCE_IO_CODE', 'nf_I0')
    const parsed = await fetchSinaHqQuote({ code, timeoutMs: 4500, fetchTextWithTimeout: params.fetchTextWithTimeout })
