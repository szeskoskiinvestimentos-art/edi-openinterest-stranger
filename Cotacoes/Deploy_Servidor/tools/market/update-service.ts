...
async function main() {
  ...
  const gitSyncTargetDir = env('MARKET_GIT_SYNC_TARGET_DIR', 'Cotacoes/dashboard/MERCADO/assets/data')
  ...
  const newsFjDiscordEnabled = envBool('NEWS_FJ_DISCORD_ENABLED', false)
  const newsDiscordBotToken = env('NEWS_DISCORD_BOT_TOKEN', '')
  const newsDiscordChannelIdRaw = env('NEWS_DISCORD_CHANNEL_ID', '')
  ...
  const telegramFinancialJuiceEnabled = envBool('TELEGRAM_FINANCIALJUICE_ENABLED', true)
  const telegramFinancialJuiceChatId = env('TELEGRAM_FINANCIALJUICE_CHAT_ID', '')
  const telegramFinancialJuiceThreadId = envIntOrNull('TELEGRAM_FINANCIALJUICE_THREAD_ID')
  const telegramFinancialJuicePollSeconds = Math.max(15, envNumber('TELEGRAM_FINANCIALJUICE_POLL_SECONDS', 120))
  const telegramFinancialJuiceMaxPerPoll = Math.max(1, Math.min(30, envNumber('TELEGRAM_FINANCIALJUICE_MAX_PER_POLL', 10)))
  const telegramFinancialJuiceBootstrapMaxItems = Math.max(0, Math.min(50, envNumber('TELEGRAM_FINANCIALJUICE_BOOTSTRAP_MAX_ITEMS', 6)))
  const telegramFinancialJuiceSentStoreEnabled = envBool('TELEGRAM_FINANCIALJUICE_SENT_STORE_ENABLED', true)
  const telegramFinancialJuiceSentRetentionDays = Math.max(1, envNumber('TELEGRAM_FINANCIALJUICE_SENT_RETENTION_DAYS', 7))
  const telegramFinancialJuiceSentStoreFile = env('TELEGRAM_FINANCIALJUICE_SENT_STORE_FILE', '')
  ...
  const fjSentStorePath = (telegramFinancialJuiceSentStoreFile && String(telegramFinancialJuiceSentStoreFile).trim())
    ? String(telegramFinancialJuiceSentStoreFile).trim()
  ...
  async function loadFjSentStore(nowMs: number) {
    if (!telegramFinancialJuiceSentStoreEnabled) return
    try {
      if (!(await fileExists(fjSentStorePath))) return
      const raw = await readFile(fjSentStorePath, { encoding: 'utf-8' })
      ...
      const list =
        parsed && typeof parsed === 'object' && 'items' in parsed && Array.isArray((parsed as Record<string, unknown>).items)
          ...
          : Array.isArray(parsed)
            ? (parsed as unknown[])
            : []
      const cutoff = nowMs - telegramFinancialJuiceSentRetentionDays * 24 * 60 * 60 * 1000
      for (const x of list) {
  ...
  async function flushFjSentStore(nowMs: number) {
    ...
    try {
      ...
      const payload = {
        updatedAt: nowISO(),
        retentionDays: telegramFinancialJuiceSentRetentionDays,
        items,
      }
      await mkdir(path.dirname(fjSentStorePath), { recursive: true })
      await writeFile(fjSentStorePath, JSON.stringify(payload, null, 2), { encoding: 'utf-8' })
      fjSentDirty = false
    } catch {
      void 0
    }
  ...
  const newsDiscordChannelId = normalizeDiscordChannelId(newsDiscordChannelIdRaw)
  ...
  async function sendTelegramOperationalOnce(opts: { reason: string; logPath?: string; force?: boolean }) {
    ...
    const payload = await buildOperationalTelegramCards({
      ...
      sourceDataDir: path.resolve(process.cwd(), String(sourceDataDir)),
  ...
  function telegramFinancialJuiceConfigured() {
    if (!telegramEnabled || !telegramFinancialJuiceEnabled) return false
    if (!telegramBotToken) return false
    if (!telegramFinancialJuiceChatId) return false
    return true
  }

  async function forwardFinancialJuiceToTelegram(opts?: { bootstrap?: boolean }) {
    ...
    const payload = await fetchJsonWithTimeout<FinancialJuiceHeadlinesPayload>(
      `${serviceBaseUrl()}/api/news/financialjuice/headlines?limit=40&onlyFj=1&t=${Date.now()}`,
      4500,
    )
    ...
    const max =
      opts && opts.bootstrap
        ? telegramFinancialJuiceBootstrapMaxItems || telegramFinancialJuiceMaxPerPoll
        : telegramFinancialJuiceMaxPerPoll
    ...
    const send = await sendTelegramMessages({
      botToken: telegramBotToken,
      chatId: telegramFinancialJuiceChatId,
      messageThreadId: telegramFinancialJuiceThreadId,
  ...
  async function fetchDiscordMessages(channelId: string, limit: number) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      const r = await fetch(url, {
        method: 'GET',
        headers: {
          Authorization: `Bot ${newsDiscordBotToken}`,
        },
      })
      if (r.status === 429) {
  ...
  function isFinancialJuiceMessage(m: DiscordMessage) {
    if (!m) return false
    if (m.author?.bot) return true
    return /financial\s*juice/i.test(String(m.author?.username || ''))
  }
  ...
  async function resolveGitRepoDir() {
    const candidates = [
      gitSyncRepoDir,
      process.cwd(),
      path.resolve(process.cwd(), '..', 'B3_System', 'Edi_OpenInterest - PY - Stranger - Indice'),
  ...
  async function gitSyncAfterUpdate(meta: { logPath: string; finishedAt: string; exitCode: number; reason: string; mode?: string }) {
    ...
    ].map(f => path.posix.join(String(sourceDataDir).replace(/\\/g, '/'), f))
    ...
    const targetDirAbs = path.resolve(repoDir, gitSyncTargetDir)
    await mkdir(targetDirAbs, { recursive: true })

    const targetFiles = sourceFiles.map(f => {
      const base = path.posix.basename(f)
      return path.posix.join(gitSyncTargetDir.replace(/\\/g, '/'), base)
    })
    ...
    for (const srcRel of sourceFiles) {
      ...
      if (!(await fileExists(srcAbs))) {
        await appendLog(meta.logPath, `GIT_SYNC error • missing source file ${srcRel}\n`)
        await finish('failed', 'missing source file')
        return
      }
      const destAbs = path.join(targetDirAbs, path.basename(srcRel))
    ...
    if (gitSyncPush) {
      ...
      if (push.exitCode !== 0) {
        ...
        await appendLog(meta.logPath, `GIT_SYNC retry • pull --rebase ${remote} ${branch}\n`)
        const pull = await spawnCapture('git', ['pull', '--rebase', remote, branch], { cwd: repoDir, env: process.env })
        if (pull.exitCode !== 0) {
          await appendLog(meta.logPath, `GIT_SYNC error • git pull --rebase failed\n${pull.stderr || pull.stdout}\n`)
  ...
  function startFinancialJuicePoller() {
    if (!telegramFinancialJuiceConfigured()) return null
  ...
  async function runDashboardExportAddons(logPath: string) {
    const args = ['run', '-s', 'market:addons']
    const res =
      process.platform === 'win32'
        ...
        : await spawnCapture('npm', args, { cwd: process.cwd(), env: { ...process.env } })
  ...
  async function runUpdate(reason: string) {
    ...
    child.on('close', code => {
      ...
      if (shutdownRequested) {
        void gitSyncAfterUpdate({ logPath, finishedAt, exitCode, reason, mode }).finally(() => {
          if (state.running) return
          if (httpServer) httpServer.close(() => process.exit(0))
          setTimeout(() => process.exit(0), 2500).unref()
        })
      } else {
        void gitSyncAfterUpdate({ logPath, finishedAt, exitCode, reason, mode })
      }
      if (exitCode === 0) {
        runDashboardExportAddons(logPath)
          .catch(() => false)
          .then(() => {
            void sendTelegramOperationalOnce({ reason, logPath }).catch(err => {
              void appendLog(logPath, `TELEGRAM operational error • ${String(err instanceof Error ? err.message : err)}\n`)
            })
          })
      }
      if (!shutdownRequested) void runScheduledIfDue()
    })
  ...
  app.get('/api/market/status', (_req, res) => {
    ...
    res.json({
      ...
      telegram: {
        ...
        operationalEnabled: telegramOperationalEnabled,
        sendOn: telegramOperationalSendOn,
        cooldownMinutes: telegramOperationalCooldownMinutes,
  ...
  app.post('/api/telegram/financialjuice/send', async (req, res) => {
    ...
    try {
      const result = await forwardFinancialJuiceToTelegram({ bootstrap })
      if (!result.ok) return res.status(400).json(result)
      res.json(result)
    } catch (err) {
  ...
  app.get('/api/news/financialjuice/headlines', async (req, res) => {
    ...
    try {
      ...
      const onlyFj = String((req.query && (req.query.onlyFj as unknown)) || '1') !== '0'
      ...
      if (!newsDiscordBotToken || !newsDiscordChannelId) {
        const payload = {
          ok: false,
          error: 'not_configured',
          generatedAt: nowISO(),
      ...
      const raw = await fetchDiscordMessages(newsDiscordChannelId, Math.max(limit, 25))
      const itemsRaw = raw
        .filter(m => {
          if (!m) return false
          if (!onlyFj) return true
          return isFinancialJuiceMessage(m)
        })
        .flatMap(m => {
      ...
      const payload = {
        ...
        url: financialJuiceUrl,
        filter: { onlyFj },
        store: { enabled: newsHeadlinesStoreEnabled, retentionDays: newsHeadlinesRetentionDays, total: merged.length },
        items: outputItems,
      }
