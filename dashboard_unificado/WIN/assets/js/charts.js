        ...
        options: {
            ...
            plugins: {
                ...
                legend: {
                    ...
                    labels: {
                        color: '#e0e0e0',
                        filter: (item) => !item.text.includes('todos strikes')
                    }
                },
                tooltip: {
                    callbacks: {
...
strangerThingsCharts.prototype.createModelsTable = function(data) {
    ...
    models.push({
        name: 'Black-Scholes', type: 'Baseline', icon: '🔵', color: '#2196f3',
    ...
    models.push({
        name: 'Heston', type: 'Stoch Vol', icon: '🟢', color: '#4caf50',
        price: () => {
    ...
    models.push({
        name: 'SVI', type: 'Vol Surface', icon: '🟣', color: '#9c27b0',
        price: () => bsCall(spot, K, T, r, ivBase * 0.97),
        delta: (S, k, t, rr, sig) => {
            if (t <= 0) return 0;
            const d1 = (Math.log(S / k) + (rr + 0.5 * sig * sig) * t) / (sig * Math.sqrt(t));
            return normCdf(d1);
    ...
    models.push({
        name: 'Dupire', type: 'Local Vol', icon: '🟠', color: '#ff9800',
        price: () => bsCall(spot, K, T, r, ivBase * 1.02),
        delta: (S, k, t, rr, sig) => {
            if (t <= 0) return 0;
            const d1 = (Math.log(S / k) + (rr + 0.5 * sig * sig) * t) / (sig * Math.sqrt(t));
            return normCdf(d1);
