...
// 13 colunas: 5 Calls + Strike + 5 Puts + 2 Net
// Logica espelhada de src/calculator/bs_model.py (Greeks) + src/data_loader.py (OI)
strangerThingsCharts.prototype.createDetailedTable = function(data) {
    ...
    // Pegar IV para um strike (interpolacao simples)
    const getIV = (K) => {
        if (ivArray.length === 0 || volSurfaceStrikes.length === 0) return 0.15;
        // Encontrar IV do strike mais proximo
        let minDiff = Infinity;
        let iv = ivArray[0];
    ...
    // Calcular ranking por OI Total (Call+Put) para mostrar top 30 mais liquidos
    const ranking = strikes.map((K, i) => ({
        idx: i,
        K,
        totalOI: (callOI[i] || 0) + (putOI[i] || 0)
    })).sort((a, b) => b.totalOI - a.totalOI).slice(0, 30);
    const sortedIndices = ranking.map(r => r.idx).sort((a, b) => a - b);

    // Encontrar strike ATM (mais proximo do spot)
    let minDiff = Infinity;
