(async function () {
  ...
  function ensureRenderLoaded() {
    ...
    const candidates = [
      ...
      '../../correlation/assets/js/supergraphics-render.js',
  ...
  const baseItems = Array.isArray(cfg && cfg.items) ? cfg.items : []
  const items = baseItems
    .filter(x => x && typeof x === 'object' && x.href)
    .map(x => ({ ...x, href: `../${String(x.href).replace(/^\.?\//, '')}` }))

  const openPopup = (href, key) => {
    ...
    window.open(href, `ediSupergraphics_${String(key || 'chart')}`, `popup=yes,width=${wpx},height=${hpx},left=${left},top=${top}`)
  }
  ...
  const nodes = items.map(it => {
    ...
    body.className = 'sg-cardbody'

    const h = document.createElement('h3')
    h.className = 'sg-cardtitle'
    h.textContent = title
    body.appendChild(h)

    const p = document.createElement('div')
    p.className = 'sg-carddesc'
    p.textContent = desc
    body.appendChild(p)
    ...
    btn.addEventListener('click', () => openPopup(href, id))
  ...
  try {
    ...
    if (ranks.length) {
      ...
      grid.replaceChildren(...ranks.map(r => r.card))
    }
  } catch {
  }
})()
