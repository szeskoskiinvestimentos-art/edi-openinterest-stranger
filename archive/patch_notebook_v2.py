...
notebook_path = 'c:/Users/ednil/Downloads/Gamma/Edi_OpenInterest/USDBRL_AnaliseDeOpcoes - Rev4.ipynb'

# Código da nova extensão (versão corrigida v2)
# Destaques:
# 1. Recalcula range_lines, wall_lines, flip_line dentro do escopo local
# 2. Usa essas variáveis locais para gerar os painéis individuais
# 3. Mantém a lógica de injeção de tabelas e thumbnails
...
# Procurar célula de extensão
found_idx = -1
for idx, cell in enumerate(nb['cells']):
    src = ''.join(cell['source'])
    if '# --- EDI EXTENSION START ---' in src:
        found_idx = idx
        break

if found_idx != -1:
    print(f"Substituindo célula de extensão encontrada no índice {found_idx}")
    nb['cells'][found_idx]['source'] = [new_extension_code]
else:
    print("Célula de extensão não encontrada. Adicionando nova célula ao final.")
    new_cell = {
        'cell_type': 'code',
        'execution_count': None,
        'metadata': {},
        'outputs': [],
        'source': [new_extension_code]
    }
...
print("Notebook atualizado com sucesso (v2).")
