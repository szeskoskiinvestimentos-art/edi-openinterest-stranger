...
cd /d "%~dp0"
title EDI Service FORCE (Opcoes + Cotacoes)
...
set "WDO_ENABLE_AUTO_GIT_PUSH=true"
set "WIN_ENABLE_AUTO_GIT_PUSH=true"
set "MARKET_GIT_SYNC_ENABLED=true"
set "MARKET_GIT_SYNC_PUSH=true"

where py >nul 2>&1
if not errorlevel 1 (
  py -3 servico_unificado.py --opcoes-agora --once %* || goto :ABORT
...
if not errorlevel 1 (
  python servico_unificado.py --opcoes-agora --once %* || goto :ABORT
