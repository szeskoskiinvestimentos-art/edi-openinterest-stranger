...
def discovery():
    logger.info("--- INICIANDO ENGENHARIA REVERSA DOS NÍVEIS ---")
    ...
    try:
        ...
        if not (os.path.exists(target_dir) and any(f.endswith('.csv') for f in os.listdir(target_dir))):
            logger.info("ERRO: Nenhum CSV encontrado em CSV_Indice configurado.")
            return
        
        df_options, spot_price, expiry = load_data(directory=target_dir, use_csv_spot=config.USE_CSV_SPOT, spot_override=config.SPOT)
        logger.info(f"Dados carregados. Spot: {spot_price}, Expiry: {expiry}")
    except Exception as e:
        logger.info(f"Erro ao carregar dados: {e}")
        return
    ...
    logger.info(f"\n--- ALVOS ---")
    logger.info(f"Put Wall Alvo: {TARGET_PUT_WALL}")
    logger.info(f"HVL Flip Alvo: {TARGET_HVL_FLIP}")
    ...
    logger.info(f"Weighted Avg OI (Top 2): {wavg_2:.4f} (Diff: {wavg_2 - TARGET_PUT_WALL:.4f})")

    # Média Ponderada Top 3 (OI)
    top3_idx = indices_oi[-3:]
    wavg_3 = np.average(strikes[top3_idx], weights=oi_put[top3_idx])
    logger.info(f"Weighted Avg OI (Top 3): {wavg_3:.4f} (Diff: {wavg_3 - TARGET_PUT_WALL:.4f})")
    ...
    logger.info(f"Weighted Avg GEX Put (Top 2): {wavg_gex_2:.4f} (Diff: {wavg_gex_2 - TARGET_PUT_WALL:.4f})")
    
    top3_gex_idx = indices_gex[-3:]
    wavg_gex_3 = np.average(strikes[top3_gex_idx], weights=put_gex[top3_gex_idx])
    logger.info(f"Weighted Avg GEX Put (Top 3): {wavg_gex_3:.4f} (Diff: {wavg_gex_3 - TARGET_PUT_WALL:.4f})")
    ...
    logger.info(f"Max Pain Strike: {max_pain:.4f} (Diff: {max_pain - TARGET_PUT_WALL:.4f})")
    ...
    logger.info(f"Raw GEX Zero Crossings indices: {idx_cross}")
    if len(idx_cross) > 0:
        for i in idx_cross:
            ...
            if y2 != y1:
                ...
                cx = x1
            logger.info(f" -> Raw GEX Flip at: {cx:.4f} (Diff: {cx - TARGET_HVL_FLIP:.4f})")
    ...
    if best_val_gauss:
        logger.info(f"Melhor Gaussian Flip: {best_val_gauss:.4f} (Diff: {best_diff_gauss:.4f}) | Sigma: {best_sigma_gauss:.2f}")
    else:
        logger.info("Nenhum Gaussian Flip encontrado.")
    ...
    if best_val_uni:
        logger.info(f"Melhor Uniform Flip: {best_val_uni:.4f} (Diff: {best_diff_uni:.4f}) | Size: {best_size_uni}")
    else:
        logger.info("Nenhum Uniform Flip encontrado.")
